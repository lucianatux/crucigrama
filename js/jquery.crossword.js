(function ($) {
  $.fn.crossword = function (entryData) {
    ...
    this.after(
      '<div id="puzzle-clues"><h2>Horizontales</h2><ul id="across"></ul><h2>Verticales</h2><ul id="down"></ul></div>'
    );
    ...
    var puzInit = {
      ...
      calcCoords: function () {
        ...
        for (var i = 0, p = entryCount; i < p; ++i) {
          ...
          for (var x = 0, j = puzz.data[i].answer.length; x < j; ++x) {
            entries[i].push(x);
            coords =
              puzz.data[i].orientation === "across"
                ? "" + puzz.data[i].startx++ + "," + puzz.data[i].starty + ""
                : "" + puzz.data[i].startx + "," + puzz.data[i].starty++ + "";
            entries[i][x] = coords;
          }
      ...
      buildEntries: function () {
        var puzzCells = $("#puzzle td"),
          ...
          hasOffset = false,
          positionOffset =
            entryCount - puzz.data[puzz.data.length - 1].position; // diff. between total ENTRIES and highest POSITIONS
    ...
    var nav = {
      ...
      updateByEntry: function (e, next) {
        ...
        if (e.keyCode === 9 || next) {
          ...
          activeClueIndex =
            activeClueIndex === clueLiEls.length - 1 ? 0 : ++activeClueIndex;
    ...
    var util = {
      highlightEntry: function () {
        ...
        $actives = $(".position-" + activePosition + " input").addClass(
          "active"
        );
        $actives.eq(0).focus();
        $actives.eq(0).select();
      ...
      highlightClue: function () {
        var clue;
        $(".clues-active").removeClass("clues-active");
        $(clueLiEls + "[data-position=" + activePosition + "]").addClass(
          "clues-active"
        );
      ...
      getActivePositionFromClassGroup: function (el) {
        ...
        if (classes.length > 1) {
          ...
          e1Ori = $(
            clueLiEls + "[data-position=" + classes[0].split("-")[1] + "]"
          )
            .parent()
            .prop("id");
          e2Ori = $(
            clueLiEls + "[data-position=" + classes[1].split("-")[1] + "]"
          )
            .parent()
            .prop("id");
          ...
          e1Cell = $(".position-" + classes[0].split("-")[1] + " input").index(
            el
          );
          e2Cell = $(".position-" + classes[1].split("-")[1] + " input").index(
            el
          );
        ...
        console.log(
          "getActivePositionFromClassGroup activePosition: " + activePosition
        );
      },
      ...
      getSkips: function (position) {
        if ($(clueLiEls[position]).hasClass("clue-done")) {
          activeClueIndex =
            position === clueLiEls.length - 1 ? 0 : ++activeClueIndex;
          util.getSkips(activeClueIndex);
        } else {
          return false;
        }
